...
class KerapuSunuDetector(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Deteksi Ikan Kerapu Sunu (6 Langkah Visualisasi)")
        # Tambahkan lebar untuk menampung 6 kolom
        self.setGeometry(100, 100, 1600, 850) 
        
        self.original_image = None
        self.processed_step_b = None 
        self.processed_step_c = None 
        self.processed_step_d = None # Mask Biner
        self.processed_step_e = None # Bintik Deteksi (Visual)
    ...
    def init_ui(self):
        ...
        # Kolom Tampilan Proses (Stretch 6) - 6 Slot Gambar
        self.image_widgets = {}
        image_titles = [
            "A. Input Citra (RGB)", 
        ...
        main_layout.addWidget(scroll_area, 6) # Stretch 6

    # Metode Utilities (Konversi dan Update Gambar)
    # ... (Dibiarkan sama) ...
    ...
    # --- FUNGSI ALGORITMA PCD UTAMA (Diperbarui untuk 6 Langkah) ---
    ...
    def process_detection(self):
        ...
        spot_detection_visual = np.zeros_like(self.original_image) # Canvas hitam
        ...
        for contour in contours:
            ...
            if is_shape_ok:
                ...
                _, spot_value_mask = cv2.threshold(v, 180, 255, cv2.THRESH_BINARY)
                ...
                spot_contours, _ = cv2.findContours(spot_value_mask_adaptif, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                ...
                for s_contour in spot_contours:
                    ...
                    if spot_area > required_min_area_per_spot and spot_area < required_max_area_per_spot:
                        ...
                        # Tandai bintik yang terdeteksi pada spot_detection_visual
                        (sx, sy, sw, sh) = cv2.boundingRect(s_contour)
                        
                        # --- Perbaikan Error Koordinat: Pastikan koordinat adalah INT ---
    ...
    # --- Fungsi Generate Laporan HTML (Diperbarui untuk 6 Langkah) ---
    ...
    def generate_report(self):
        ...
        img_c = self.cv_to_base64(self.processed_step_c) # Mask Biner
        img_d = self.cv_to_base64(self.processed_step_d) # Masked Fish
        img_e = self.cv_to_base64(self.processed_step_e) # Bintik Visual
        img_f = self.cv_to_base64(self.detected_img)     # Hasil Akhir
        ...
        <body>
            <div class="container">
                ...
                <div class="step">
                    <h2>3. Segmentasi Warna Ikan (Mask Biner) (C)</h2>
                    <div class="step-content">
                        <div class="analysis"><strong>Analisa:</strong> Citra diolah di ruang warna **HSV** untuk mengisolasi warna merah/oranye ikan, menghasilkan **Mask Biner** (putih = ikan, hitam = background).</div>
                ...
                <div class="step">
                    ...
                    <div class="step-content">
                        <div class="analysis"><strong>Analisa:</strong> Mask Biner diterapkan pada citra RGB asli. Hasilnya adalah ikan yang terisolasi dengan *background* hitam, siap untuk analisis bentuk dan tekstur bintik.</div>
                ...
                <div class="step">
                    ...
                    <div class="step-content">
                        <div class="analysis">
                            <strong>Analisa:</strong> Bintik dideteksi berdasarkan **Kecerahan Tinggi (Value > 200)**. Setiap kontur yang memenuhi kriteria ukuran (berdasarkan persentase area ikan) ditandai dengan kotak merah.
                        </div>
                ...
                <div class="step">
                    ...
                    <div class="step-content">
                        <div class="analysis">
                            ...
                            <strong>Kriteria Deteksi:</strong> Bentuk (Circularity & Aspect Ratio) cocok DAN Total Area Bintik Terang melebihi **{MIN_TOTAL_SPOT_AREA_PERCENT}%** dari area ikan.
                        </div>
